package co.app.food.andromeda.shadow
...
import co.app.food.andromeda.extensions.getEnum
...
class AndromedaShadowLayout @JvmOverloads constructor(
    ...
    init {
        ...
        readAttributes(attributeSet, R.styleable.AndromedaShadowLayout) {
            ...
            shadowType = it.getEnum(
                R.styleable.AndromedaShadowLayout_sl_type,
                ShadowType.values(),
                shadowType
            )
            shadowRegion = shadowRegion.copy(
                left = it.getBoolean(
                    R.styleable.AndromedaShadowLayout_enable_shadow_left,
                    shadowRegion.left
                ),
                top = it.getBoolean(
                    R.styleable.AndromedaShadowLayout_enable_shadow_top,
                    shadowRegion.top
                ),
                right = it.getBoolean(
                    R.styleable.AndromedaShadowLayout_enable_shadow_right,
                    shadowRegion.right
                ),
                bottom = it.getBoolean(
                    R.styleable.AndromedaShadowLayout_enable_shadow_bottom,
                    shadowRegion.bottom
                )
            )
            ...
            val shadowEnabled =
                it.getBoolean(R.styleable.AndromedaShadowLayout_shadow_enabled, true)
            if (!shadowEnabled) {
                shadowRadius = 0f
            }
        }
    ...
    private fun updateShadowParameters(shadowType: ShadowType) {
        when (shadowType) {
            ShadowType.LOW -> {
                originalShadowRadius = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_low_blur
                )
                dx = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_low_x
                )
                dy = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_low_y
                )
                shadowColor = AndromedaAttributeManager.getColorFromAttribute(
                    context,
                    R.attr.shadow_low_color
                )
            }
            ShadowType.HIGH -> {
                originalShadowRadius = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_high_blur
                )
                dx = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_high_x
                )
                dy = AndromedaAttributeManager.getDimensionFromAttribute(
                    context,
                    R.attr.shadow_high_y
                )
                shadowColor = AndromedaAttributeManager.getColorFromAttribute(
                    context,
                    R.attr.shadow_high_color
                )
            }
        }
        shadowRadius = originalShadowRadius
