package co.app.food.andromeda.button
...
import co.app.food.andromeda.databinding.AndromedaButtonBinding
import co.app.food.andromeda.exception.ColorTokenException
import co.app.food.andromeda.extensions.*
import co.app.food.andromeda.tokens.spacing_3x
import co.app.food.andromeda.tokens.spacing_4x
import co.app.food.andromeda.tokens.spacing_x
...
class AndromedaButton @JvmOverloads constructor(
    ...
    private fun getBackgroundColorStateListDrawable(
        ...
        bgDrawableActive.cornerRadius = bgRadius.toPxFloat(context)
        val bgDrawablePressed =
            context.getDrawableCompat(drawableRes)!!.mutate() as GradientDrawable
        bgDrawablePressed.cornerRadius = bgRadius.toPxFloat(context)
        val bgDrawableInactive =
            context.getDrawableCompat(drawableRes)!!.mutate() as GradientDrawable
        bgDrawableInactive.cornerRadius = bgRadius.toPxFloat(context)
    ...
    private fun getBackgroundStrokeStateListDrawable(
        ...
        bgDrawableNormal.cornerRadius = bgRadius.toPxFloat(context)
        val bgDrawableInactive =
            context.getDrawableCompat(drawableRes)!!.mutate() as GradientDrawable
        bgDrawableInactive.cornerRadius = bgRadius.toPxFloat(context)
    ...
    private fun setButtonBackground(@DrawableRes drawableRes: Int) {
        when (buttonType) {
            ButtonType.PRIMARY_POSITIVE_REGULAR,
            ButtonType.PRIMARY_POSITIVE_TINY,
            ButtonType.PRIMARY_POSITIVE_FULL_WIDTH -> {
            ...
            ButtonType.PRIMARY_DESTRUCTIVE_REGULAR,
            ButtonType.PRIMARY_DESTRUCTIVE_TINY,
            ButtonType.PRIMARY_DESTRUCTIVE_FULL_WIDTH -> {
                val activeColor = AndromedaAttributeManager.getColorFromAttribute(
                    context,
    ...
    fun setDebouncedClickListener(buttonClickedListener: (() -> Unit)? = null) {
        findViewById<LinearLayout>(R.id.ll_button_container).setOnClickListener(object :
        ...
        if (isLoading) {
            findViewById<LinearLayout>(R.id.ll_button_container).isClickable = false
        }
    }
